...
module Jekyll
  ...
  class Img < Liquid::Tag
    ...
    def render(context)
      ...
      raise "Image Tag can't read this tag. Try {% img [preset or WxH] path/to/img.jpg [attr=\"value\"] %}." unless markup
      ...
      nolink = html_attr.key?('nolink')
      html_attr.delete('nolink')
      alt = html_attr.delete('alt')
      title = html_attr.delete('title')
      html_attr['class'] += " image-container"
      title_html = title ? "title='#{title}'" : ''
      alt_html = alt ? "alt='#{alt}'" : ''
      ...
      full_size_src = Pathname.new(File.join('/', settings['output'], instance[:src])).cleanpath

      # Return the markup!
      output = ''
      output += "<div #{html_attr_string}>"
      output += "<a href=\"#{full_size_src}\" class=\"image-border\" #{title_html}>" unless nolink
      output += "<span #{title_html}>" if nolink
      output += "<img src=\"#{generated_src}\" #{alt_html}/>"
      output += "</span>" if nolink
      output += '</a>'unless nolink
      output += '</div>'
      output
    end
